...
A2A_RPC_URL = BASE_URL + "a2a/dev-agent/"
...
def test_a2a_agent_json_generation(server_fixture: subprocess.Popen[str]) -> None:
    ...
    # Validate agent card quality (post-processing fixes)
    assert served_agent_card["version"] != "0.0.0", "Version should not be placeholder"

    # Provider metadata should be present
    assert served_agent_card.get("provider"), "Agent card should include provider info"

    # Root skill should have a concise description, not the full system prompt
    skills = served_agent_card["skills"]
    root_skill = next((s for s in skills if s["id"] == "dev_agent"), None)
    assert root_skill is not None, "Root skill 'dev_agent' not found"
    assert root_skill["name"] != "model", (
        "Root skill name should be descriptive, not 'model'"
    )
    assert len(root_skill["description"]) < 500, (
        f"Root skill description too long ({len(root_skill['description'])} chars) "
        "— likely contains the full system prompt instead of a concise summary"
    )
    assert root_skill.get("examples"), "Root skill should have example prompts"
