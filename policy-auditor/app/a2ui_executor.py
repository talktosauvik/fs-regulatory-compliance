...
class A2UIAgentExecutor(agent_execution.AgentExecutor):
    ...
    async def execute(
        ...
        if _A2UI_DELIMITER not in final_response_text:
            ...
            # Parse and validate A2UI JSON securely
            json_str = json_section.strip()
            
            # Safely remove markdown code blocks using exact string matching
            json_str = (
