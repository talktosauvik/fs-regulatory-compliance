...
# ---------------------------------------------------------------------------
# Tool: Read Remediation Spec from GCS
# ---------------------------------------------------------------------------


async def read_remediation_spec_from_gcs(
    filename: str,
) -> str:
    """Reads the IT remediation JSON specification from the GCS handoff bucket.

    Use this tool to retrieve the remediation tasks and requirements generated
    by the Policy Auditor.

    Args:
        filename: The name of the JSON file to read (e.g. 'SEC_compliance_portal_spec.json').

    Returns:
        The JSON content of the specification as a string, or an error message.
    """
    bucket_name = os.getenv("FSI_DEV_HANDOFF_BUCKET") or os.environ.get("ASSET_BUCKET_NAME", "")
    if not bucket_name:
        return "❌ Error: FSI_DEV_HANDOFF_BUCKET or ASSET_BUCKET_NAME environment variable is not configured."
    ...
    try:
        ...
        if not blob.exists():
            return f"❌ Error: File '{filename}' not found in bucket '{bucket_name}'."
        
        content = blob.download_as_text()
        return content
    except Exception as e:
        logger.exception("Failed to read remediation spec from GCS")
        return f"❌ Error reading spec from GCS: {e}"



# ---------------------------------------------------------------------------
# Tool: Create Batched Remediation Tickets from JSON Spec
# ---------------------------------------------------------------------------


async def create_batched_remediation_tickets(
    spec_json_content: str,
    tool_context: ToolContext | None = None,
) -> str:
    """Creates individual Jira tickets for each remediation task in the JSON specification.
    
    Use this tool ONLY to batch-create Jira tickets from the retrieved remediation JSON.

    Args:
        spec_json_content: The raw JSON string containing the remediation tasks.
    """
    # --- SKIP_JIRA: create GCS-based tasks instead ---
    if SKIP_JIRA:
        logger.info("SKIP_JIRA is enabled — creating GCS-based tasks in batch.")
        if not _TASK_BUCKET:
            return "❌ Error: ASSET_BUCKET_NAME is not configured. Cannot create tasks."
            
        try:
            data = json.loads(spec_json_content)
            tasks = data.get("remediation_tasks", [])
        except Exception as e:
            return f"❌ Error: Failed to parse JSON specification: {e}"
            
        created_ids = []
        for task in tasks:
            task_id = task.get("task_id", "N/A")
            summary = f"[{task.get('regulation', 'FSI')}] Compliance Task: {task_id}"
            ...
            try:
                await asyncio.to_thread(_upload_task_json, task_id, task_data)
                created_ids.append(task_id)
            except Exception as e:
                logger.exception("Failed to upload GCS task JSON")
                
        if tool_context is not None:
            tool_context.state["_jira_ticket_created"] = ", ".join(created_ids)
            
        return f"✅ **Batch GCS Tasks Created Successfully**: {', '.join(created_ids)}"
    ...
    if not tasks:
        return "❌ Error: No remediation tasks found in the provided specification."

    email = os.environ.get("JIRA_EMAIL") or os.environ.get("JIRA_USER_EMAIL") or "elhadik@google.com"
    api_token = os.environ.get("JIRA_API_TOKEN")
    jira_url = os.environ.get("JIRA_BASE_URL") or "https://google-team-vwhbosar.atlassian.net"
    ...
    for task in tasks:
        regulation = task.get("regulation", "N/A")
        compliance_area = task.get("compliance_area", "N/A")
        task_id = task.get("task_id", "N/A")
        desc = task.get("description", "")
        priority_val = task.get("priority", "Medium").lower()
        due_date = task.get("due_date", "")
        assigned_to = task.get("assigned_to", "N/A")

        summary = f"[{regulation}] FSI Compliance: {task_id}"
        description = (
            f"{desc}\n\n"
            f"--- Task Metadata ---\n"
            f"• Regulation: {regulation}\n"
            f"• Compliance Area: {compliance_area}\n"
            f"• Assigned To Team: {assigned_to}\n"
            f"• Targeted Due Date: {due_date}"
        )

        jira_priority = priority_map.get(priority_val, "Medium")
        ...
        try:
            issue = await asyncio.to_thread(jira.issue_create, fields=fields)
            ticket_key = issue.get("key", "UNKNOWN")
            created_tickets.append((task_id, ticket_key))
            logger.info("Successfully created Jira ticket %s for task %s", ticket_key, task_id)
        except Exception as e:
            logger.exception("Failed to create ticket for task %s", task_id)
            created_tickets.append((task_id, f"Error: {e}"))

    # Format results
    output_lines = ["🎫 **Batched Remediation Tickets Created**:\n"]
    success_keys = []
    for task_id, ticket_key in created_tickets:
        if ticket_key.startswith("Error"):
            output_lines.append(f"❌ Task **{task_id}**: {ticket_key}")
        else:
            browse_url = f"{jira_url.rstrip('/')}/browse/{ticket_key}"
            output_lines.append(f"• Task **{task_id}** $\\rightarrow$ [{ticket_key}]({browse_url})")
            success_keys.append(ticket_key)
            
    if tool_context is not None and success_keys:
        tool_context.state["_jira_ticket_created"] = ", ".join(success_keys)
...
async def create_jira_ticket(
    ...
    """Creates a support ticket in Jira (or a local task when Jira is disabled).

    Use this tool to file engineering tasks, bug reports, or feature requests.

    Args:
        summary: A concise title for the ticket.
        description: A detailed description of the issue/discrepancy.
        project_key: The Jira project key to file the ticket under. Defaults to "SCRUM".
        issue_type: The type of issue to create (e.g. "Task"). Defaults to "Task".
    ...
    if tool_context is not None:
        ...
        if created_key and created_key != "__pending__":
            return (
                f"⚠️ A ticket was already created in this conversation: {created_key}."
            )
    ...
    try:
        issue = await asyncio.to_thread(_call_jira_api)
        ticket_key = issue.get("key", "UNKNOWN")
        browse_url = f"{jira_url.rstrip('/')}/browse/{ticket_key}"
...
def _before_tool_callback(
    ...
    """
    if tool.name in ("create_jira_ticket", "create_batched_remediation_tickets"):
...
root_agent = Agent(
    ...
    name="fs_jira_agent",
    description=(
        "A developer assistant agent that communicates with dev teams via Google Chat, "
        "creates and manages Jira support tickets for FSI compliance remediation tasks."
    ),
    ...
    tools=[
        ...
        get_campaign_videos,
        create_jira_ticket,
        create_batched_remediation_tickets,
        get_jira_ticket,
        start_jira_ticket,
        read_remediation_spec_from_gcs,
    ],
)
