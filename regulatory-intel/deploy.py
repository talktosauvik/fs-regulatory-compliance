"""Deploy script for Regulatory Intel to Agent Engine and Gemini Enterprise."""
...
def main():
    parser = argparse.ArgumentParser(description="Deploy Regulatory Intel to Agent Engine")
    parser.add_argument("--display_name", default="Regulatory Intel", help="GE Display Name")
    parser.add_argument("--engine_id", default=None, help="If provided, updates this existing Agent Engine ID instead of creating a new one.")
    ...
    print("=" * 80)
    if args.engine_id:
        print(f"🚀 UPDATING Regulatory Intel on Agent Engine (ID: {args.engine_id})...")
    else:
        print("🚀 CREATING NEW Regulatory Intel on Agent Engine...")
    ...
    from app.agent import root_agent, RegulatoryExecutor
    
    # 3. Manually build the Agent Skill
    skill_intel = AgentSkill(
        id="Regulatory_Intel_Agent",
        name="Regulatory Intel Analyst",
        description=root_agent.description,
        tags=["compliance", "regulatory", "sec", "finra"],
        examples=[
            "Extract the cybersecurity disclosure timeline from SEC Rule 33-11216.",
            "What are the FINRA requirements for third-party vendor cyber-fraud monitoring?",
        ],
    )

    # 4. Create the Agent Card
    agent_card = create_agent_card(
        agent_name="Regulatory_Intel_Agent",
        description=root_agent.description,
        skills=[skill_intel],
    ...
    # 5. Use the A2aAgent wrapper
    a2a_agent = A2aAgent(
        agent_card=agent_card,
        agent_executor_builder=RegulatoryExecutor,
    )
    a2a_agent.set_up()
    ...
    keys_to_pass =[
        ...
        "LOGS_BUCKET_NAME", "FSI_EXTERNAL_REGS_BUCKET", "AGENT_VERSION"
    ]
    ...
    # 7. Create or Update using client.agent_engines
    if args.engine_id:
    ...
    # 8. Fetch A2A Card from the LIVE endpoint
